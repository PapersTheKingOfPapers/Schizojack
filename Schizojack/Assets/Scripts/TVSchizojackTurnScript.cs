...
public class TVSchizojackTurnScript : MonoBehaviour
{
    ...
    void Update()
    {
        if(_prevCurrentTurn != SB._currentTurn && _active)
        {
            ...
            {
                var temp = tvScreens[i].GetComponent<Renderer>().materials;
                temp[1].enableInstancing = true;
                temp[1] = (i == SB._currentTurn) ? tvScreenYTMat : tvScreenWMat;
                tvScreens[i].GetComponent<Renderer>().materials = temp;
